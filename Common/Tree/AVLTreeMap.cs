...
namespace Common.Tree
{
    ...
    public class AVLTreeMap<K,V> : IMap<K, V> where K : IComparable
    {
        ...
        {
            public TreeNode(K key,V value, TreeNode left = null, TreeNode right = null)
            {
                Left = left;
                Right = right;
                Key = key;
        ...
        /// <param name="value">元素值</param>
        /// <returns></returns>
        private TreeNode AddNode(TreeNode node,K key ,V value)
        {
            if (node == null)
        ...
        {
            Root = SuccessorRemove(Root, key);
        }

        private TreeNode SuccessorRemove(TreeNode node, K key)
        {
            if (node == null)
                return default;
            ...
            {
                node.Left = SuccessorRemove(node.Left, key);
                retNode = node;
            }
            else if (node.Key.CompareTo(key) < 0)
            {
                node.Right = SuccessorRemove(node.Right, key);
                retNode = node;
            }
            else
            {
                ...
                else
                {
                    ...
                    successorNode.Right = SuccessorRemove(node.Right, successorNode.Key);
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        private TreeNode Find(TreeNode node,K key)
        {
        ...
        private K FindMaxValue(TreeNode node)
        {
            if (node.Right == null)
                return node.Key;
        ...
        private K FindMinValue(TreeNode node)
        {
            if (node.Left == null)
                return node.Key;
        ...
        private bool Contains(TreeNode node, K key)
        {
            if (node == null)
                return false;
        ...
        private void InOrder(TreeNode node, ref List<K> nums)
        {
            if (node == null)
                return;
